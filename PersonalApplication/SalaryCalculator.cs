    ...
    public partial class SalaryCalculator : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else if (cmbTitle.SelectedItem.ToString() == "IT Manager(maks 5 personel)")
                {
                ...
                if (user.KullaniciTipleri == "Admin" || user.KullaniciTipleri == "*Admin" || user.KullaniciTipleri == "admin" || user.KullaniciTipleri == "user" || user.KullaniciTipleri == "User")
                {
                    MaasBuilder vBuilder = new KullaniciMaasBuilder();
                    vBuilder.Maas.BaslangicMaas = initialSalary;
                    vBuilder.Maas.Sehir = city;
                    vBuilder.Maas.Tecrube = experience;
                    vBuilder.Maas.AileDurmu = Family_Status;
                    vBuilder.Maas.YabanciDil = foreign_Language_Knowledge;
                    vBuilder.Maas.Egitim = higher_education;
                    vBuilder.Maas.GorevYonetimi = management_Task;
                    MaasHesaplayici director = new MaasHesaplayici();
                    double maas = director.Calculate(vBuilder);
                    txtBoxSalary.Text = maas.ToString();
                }
                else
                {
                    MaasBuilder vBuilder = new PartTimeMaasBuilder();
                    vBuilder.Maas.BaslangicMaas = initialSalary;
                    vBuilder.Maas.Sehir = city;
                    vBuilder.Maas.Tecrube = experience;
                    vBuilder.Maas.AileDurmu = Family_Status;
            ...
            {
                MessageBox.Show("Lütfen Bütün Boşlukları Doldurun", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                if (LoginForm.userList[i].KullaniciAdi == user.KullaniciAdi)
                    LoginForm.userList[i].KullaniciAdi = txtBoxSalary.Text;
            }
            LoginedUser.getInstance().UserGetSet.Maas = txtBoxSalary.Text;
