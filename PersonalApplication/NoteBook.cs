...
namespace Personal_Organizer_Application
{
    public partial class NoteBook : Form
    {
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (txtNote.Text == "")
            {
                MessageBox.Show("Boş Bir Not Kaydedilemez.Lütfen Girdi girin.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                counter++;
            }
            if (counter != 1)
            {
                Notes note = new Notes(user.KullaniciAdi, txtNote.Text);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Util.SaveNotebook(listNotebook, path);
                MessageBox.Show("Not Silindi!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                List();
                txtNote.Text = "";
            }
            catch (Exception)
